...
local ChannelButtons = {
    {name = "say", text = "说", color = {0.90, 1.00, 1.00}, callback = ChannelSay_OnClick},
    ...
    {name = "world", text = "世", color = {1.00, 0.71, 0.76}, callback = ChannelWorld_OnClick},
    {name = "emote", text = "表", color = {1.00, 1.00, 0.60}, callback = ChatEmote_OnClick},
    {name = "roll", text = "R", color = {1.00, 1.00, 0.00}, callback = Roll_OnClick},
...
local function CreateChannelButton(data, index)
    ...
    if UseVertical then
        ...
        frame:SetPoint("LEFT", chat, "LEFT", 10 + (index - 1) * 20, 0)-- 锚点 30为间距
    end
    
    frame:RegisterForClicks("AnyUp")
    frame:SetScript("OnClick", data.callback)
    frameText = frame:CreateFontString(data.name .. "Text", "OVERLAY")
    frameText:SetFont("fonts\\ARHei.ttf", 14,"OUTLINE")-- 字体设置
	...
	--frameText:SetTextColor(0.90,1,1)
end
